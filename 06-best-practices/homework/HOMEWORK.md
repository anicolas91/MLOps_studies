## Homework
We will take the ride prediction model from module 4 and create tests.

## Q1. Refactoring

We refactor the code by getting rid of all the global variables. 
...
The main block of the script needs to have:
```python
if __name__=="__main__":
```

And inside that if statement should be your code to run.
...
The other file needed is `__init__.py`, without it you cannot indicate that
it is a library and thus cannot import it.
...
there should be only 2, the other two are too short and get chucked out during cleanup.
